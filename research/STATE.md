...
Can an exact local `d=4 <-> d=5` Lorentzian theory be defined without a physical
ambient bulk, from one covariant first-order variational principle, and shown
provisionally consistent? The active F0 discriminator is now narrower: does the
enriched fixed-interface sector have only first-class zero-charge gauge data, or
does it retain physical/second-class edge structure?

## Current working picture

- `D002`–`D005` show that the bare quotient has useful intrinsic kinematics but
  cannot by itself supply the measure/incidence and boundary momentum matching
  needed for Palatini dynamics. A nondegenerate fixed-interface branch requires
  genuine reference/edge enrichment (C007–C015).
- `D006` completed the bounded `2+1` analysis of that interface/edge action on
  the planar rank-three basic persistent `SO(1,3)` branch. The fixed-reference
...
- The advertised H002-versus-restricted-H003 classification remains
  **inconclusive**. D006 lacks the bulk symplectic form, total Lorentz Gauss
  ...
  whether the rank-ten block is physical, second class, or removed by
  first-class total gauge.
...
- **H001 — intrinsic stratified continuum dynamics:** strong autonomous form
  excluded for the bare quotient; survives only with declared incidence data.
- **H002 — resolution-based gauge-equivalent dynamics:** pure-gauge form is
  disfavored. It survives only if a future total bulk-plus-interface generator
  removes the C016 residual block with vanishing admissible charge.
- **H003 — continuum rank-change obstruction:** restricted form is supported;
  strong nonexistence is not established because D005 has a nondegenerate
  enriched branch. C016 is a warning in its favor, not a proof.
- **H004/H005 — Hopf spin-statistics competitors:** unchanged and downstream of
  F0; they must not influence action design.
...
- Quotient topology, smooth structure, ambient horn calculus, and final quotient
  calculus are distinct and must not be silently combined.
- Coframe omission, forgetful equivalence, ordinary gauge, and dynamical
  exclusion remain inequivalent operations.
- The D005 repair is profile-independent but adds a marked circle, edge map,
  multipliers, scale, and reference data; its physical legitimacy is unresolved.
- Exact machine checks are faithful only to their declared finite
  representations and assumptions; they do not establish smooth/global
  quantifiers, bulk compatibility, constraint closure, or hyperbolicity.

No ledger claim is currently marked contradicted. The listed tensions are open
category or interpretation conflicts and must not be averaged away.

## Literature gaps

1. Hamiltonian edge-mode and corner-charge classifications for first-order
   gravity with a covariantly transforming reference connection.
2. A bulk-plus-interface Dirac analysis for degenerate-coframe rank change.
3. Moving-stratum variation with corner terms in mixed-dimensional actions.
4. Integration/fundamental-class constructions on the non-pure quotient with a
   disconnected collapsed link, including direct `5 -> 3` incidence.
...
3. Return to C004–C006 and Hopf-sector work only after the F0 action and
   constraint problem is resolved.
