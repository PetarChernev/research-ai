...
An experiment is not a machine-check obligation. An experiment explores a hypothesis or computes an observable; an obligation under `research/checks/` tests one concrete declared assertion against a predeclared acceptance criterion and has its canonical outcome written only by `scripts/run_check.py`. The same computation may motivate both artifacts, but neither substitutes for the other. Reusable machinery shared between them belongs in `research/computation/`.
