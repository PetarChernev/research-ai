...
LEGACY_VERIFICATION_CUTOFF = datetime.fromisoformat("2026-08-22").date()
CRITIQUE_OUTCOMES = {
...
LEGACY_VERIFICATION_SECTIONS = [
    section for section in VERIFICATION_SECTIONS if section != "Scope and decisive bridge"
]
CRITIQUE_SECTIONS = [
...
def verification_report_is_substantive(
    body: str, required_sections: list[str] = VERIFICATION_SECTIONS
) -> bool:
    sections = markdown_sections(body)
    ...
    for index, entry in enumerate(entries):
        ...
        for source in evidence.get("verification", []) if isinstance(evidence.get("verification"), list) else []:
            if not isinstance(source, str):
                ...
                if (
                    outcome == "verified"
                    and report_metadata.get("legacy") is not True
...
def validate_verification_reports(root: Path, report: Report, claims: set[str]) -> None:
    ...
    for path in directory.rglob("*.md"):
        ...
        try:
            ...
            report.error(location, str(exc))
            continue
        legacy = metadata.get("legacy") is True
        if "legacy" in metadata and not isinstance(metadata.get("legacy"), bool):
            report.error(location, "legacy must be a boolean")
        required_fields = {
            "claim_id",
            "outcome",
            "date",
        ...
        if not legacy:
            required_fields.add("user_approved")
        require_fields(metadata, required_fields, location, report)
        ...
        report_date = None
        try:
            report_date = datetime.fromisoformat(str(metadata.get("date"))).date()
        except ValueError:
            report.error(location, "date must be ISO-8601")
        verifier = metadata.get("verifier")
        verifier_model = metadata.get("verifier_model")
        if legacy:
            if report_date is not None and report_date >= LEGACY_VERIFICATION_CUTOFF:
                report.error(location, "legacy verification reports must predate 2026-08-22")
            if (
                ...
                report.error(location, "legacy verifier must identify the actual agent or method")
            if not valid_model_id(verifier_model):
                report.error(location, "legacy verifier_model must be a full provider/model ID")
        else:
        ...
        if not isinstance(originating_models, list) or not originating_models:
            ...
            for model in originating_models:
                if model == "unknown":
                    ...
                    report.error(location, f"invalid originating model ID '{model}'")
            if valid_model_id(verifier_model) and not legacy:
        ...
        required_sections = LEGACY_VERIFICATION_SECTIONS if legacy else VERIFICATION_SECTIONS
        require_sections(body, required_sections, location, report)
        if not verification_report_is_substantive(body, required_sections):
