...
Producer models in this workspace are configured per agent and need not match the director's model. Record your own full `provider/model` ID in the experiment's provenance and state it in your final message, so the director can route verification to a model that did not produce the work.
