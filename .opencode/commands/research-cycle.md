...
7. If an important new claim is conclusion-critical, select the model-bound verifier from a different provider: `verifier-anthropic` for OpenAI-originated evidence or `verifier-openai` for Anthropic-originated evidence. Pass the exact claim, primary artifacts including computational contracts, environment manifests, Engineer-authored infrastructure and tests, obligation specs, claim-specific implementations, infrastructure fingerprints, machine results, and every material Scientific Computation and Engineer model ID. If provenance is unknown or mixed across both verifier models, disclose that limitation and do not call the result model-independent.
