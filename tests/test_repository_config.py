...
class ArtifactSurfaceTests(unittest.TestCase):
    ...
    def test_provenance_execution_detection_does_not_match_path_inspection(self) -> None:
        text = (PROJECT_ROOT / ".opencode" / "plugins" / "research-provenance.js").read_text(
            encoding="utf-8"
        )
        self.assertIn(r"scripts\/run_check\.py\s+(O\d{3})", text)
        self.assertIn(r"research\/experiments\/(E\d{3})\/run\.py", text)
        self.assertNotIn(r"(?:research\/checks\/|run_check\.py", text)
        self.assertNotIn(r"command.match(/research\/experiments\/", text)
